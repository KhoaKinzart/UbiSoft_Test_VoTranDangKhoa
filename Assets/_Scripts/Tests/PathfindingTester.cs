...
public class PathfindingTester : MonoBehaviour
{
    ...
    // L?u ?i?m b?t ??u v� k?t th�c (T?a ?? Grid)
    ...
    void Start()
    {
        // M?c ??nh ch?n ?i?m (0,0) v� (1,1) ?? test
    ...
    void Update()
    {
        ...
        // 2. Nh?n Input chu?t ?? test cho l? (Tr�i: Start, Ph?i: End)
        HandleMouseInput();
    }

    private void InitializeEngine()
    {
        // Kh?i t?o A* v?i k�ch th??c th?t c?a Map
        aStarEngine = new AStar(gridManager.CurrentWidth, gridManager.CurrentHeight);

        // N?p d? li?u v?t c?n v�o A*
    ...
    {
        if (Input.GetMouseButtonDown(0)) // Chu?t tr�i -> Ch?n ?i?m b?t ??u
        {
            SetCoordinateFromMouse(ref startCoord);
            RecalculatePath();
        }
        else if (Input.GetMouseButtonDown(1)) // Chu?t ph?i -> Ch?n ?�ch ??n
        {
    ...
    void OnDrawGizmos()
    {
        ...
        if (currentPath != null && currentPath.Count > 1)
        {
            ...
            for (int i = 0; i < currentPath.Count - 1; i++)
            {
                ...
                Vector3 posA = new Vector3(nodeA.x, 1f, nodeA.y); // V? cao l�n Y=1 cho d? th?y
                Vector3 posB = new Vector3(nodeB.x, 1f, nodeB.y);

                Gizmos.DrawLine(posA, posB);
                Gizmos.DrawSphere(posA, 0.2f); // V? th�m c?c tr�n nh? ? m?i b??c
            }
        }
    }
}